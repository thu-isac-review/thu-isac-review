...
function bindEvents(container) {
    ...
    ['country', 'city', 'industry', 'venue'].forEach(type => {
        ...
        container.querySelector(`#search-${type}-input`)?.addEventListener('keyup', (e) => {
            ...
            const labels = document.getElementById(`${type}-options-container`).querySelectorAll('.filter-option');
            labels.forEach(lbl => {
    ...
    container.querySelector('#btn-close-modal-x')?.addEventListener('click', () => document.getElementById('view-modal')?.classList.remove('open'));
    container.querySelector('#btn-close-modal')?.addEventListener('click', () => document.getElementById('view-modal')?.classList.remove('open'));

    // 表格本體點擊 (展開分支、點整行直接查看詳細資料)
    container.querySelector('#table-body')?.addEventListener('click', (e) => {
        ...
        // 如果是點擊查看詳細按鈕，或是點擊行本身(且不是點到 tree toggle 鍵)
        if (actionBtn || tr) {
...
async function fetchInitialDataOnce() {
    try {
        ...
        console.error(e);
        document.getElementById('table-body').innerHTML = `<tr><td colspan="8" style="text-align:center; padding:40px; color:var(--danger);">資料載入中斷，請確認您的帳號權限或刷新重試。</td></tr>`;
    }
}
...
function openDetailModal(id) {
    ...
    document.getElementById('view-parent-name').textContent = parentObj ? parentObj.name : '獨立主機構 (無隸屬)';
    ...
    const wrapTrans = document.getElementById('wrap-translated-view');
    if (!isDomestic && docData.name_translated) {
        if(wrapTrans) wrapTrans.style.display = 'block';
        document.getElementById('view-name-translated').textContent = docData.name_translated;
    } else if(wrapTrans) {
        wrapTrans.style.display = 'none';
    }
    
    document.getElementById('view-address').textContent = docData.address || '-';
    document.getElementById('view-industry').textContent = docData.industry || '未填寫';
    document.getElementById('view-venue-type').textContent = docData.venue_type || '未填寫';
    document.getElementById('view-remarks').textContent = docData.remarks || '無備註說明。';
    
    document.getElementById('view-modal')?.classList.add('open');
}

function renderHistoryList() {
    const container = document.getElementById('history-list-container-view');
...
function updateColStyles() {
    ...
    const dynStyle = document.getElementById('dynamic-col-styles-view');
